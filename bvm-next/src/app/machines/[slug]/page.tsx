...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
    ...
    return {
        title: `${machine.hero.title} | BVM Industries`,
        description: machine.hero.description,
        openGraph: {
            images: [machine.hero.image],
        }
    };
}

export default async function MachineDetailsPage({ params }: { params: Promise<{ slug: string }> }) {
    const { slug } = await params;
    const machine = machines.find(m => m.slug === slug);
            ...
            <RevealSection className="px-4 sm:px-8 lg:px-[8vw] mb-20">
                ...
                <div className="flex flex-wrap items-center gap-4">
                    ...
                    {(machine.slug === 'ffs' || machine.slug === 'bfs') && (
                        <Link
                            href="/contact?intent=ga-drawing"
                            className="bg-bvm-blue/10 hover:bg-bvm-blue text-bvm-blue hover:text-white border border-bvm-blue/50 px-6 py-3 rounded-lg font-medium transition-colors flex items-center gap-2"
                        >
                            <span>Download G.A. Drawing (PDF)</span>
                            <ArrowRight className="w-4 h-4" />
                        </Link>
                    )}
                </div>
