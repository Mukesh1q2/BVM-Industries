      ...
      >
        <div className="max-w-[1920px] w-full mx-auto px-4 sm:px-8 flex items-center justify-between">
            ...
            {navStructure.map((item) => (
              ...
              >
                <div className="px-3 py-2 cursor-pointer group flex items-center gap-1">
                  {item.href ? (
                    <Link href={item.href} className="text-sm font-medium transition-colors uppercase tracking-wide text-gray-700 dark:text-white/90 hover:text-bvm-blue" onClick={() => setActiveDropdown(null)}>
                      {item.label}
                    </Link>
                  ) : (
                    <button
                  ...
                  {/* Active Indicator */}
                  <span className={`absolute bottom-0 left-3 right-3 h-0.5 bg-bvm-blue scale-x-0 transition-transform duration-300 origin-left ${activeDropdown === item.label || (item.href && location === item.href) ? 'scale-x-100' : 'group-hover:scale-x-100'}`} />
                </div>
                            ...
                            <ul className="space-y-3">
                              {col.items.map((subItem) => (
                                <li key={subItem.label}>
                                  <Link href={subItem.href} className="group/item block" onClick={() => setActiveDropdown(null)}>
                      ...
                      <div className="bg-white/95 dark:bg-bvm-navy/95 backdrop-blur-2xl border border-gray-200 dark:border-white/10 rounded-xl p-4 shadow-xl min-w-[220px] animate-in fade-in slide-in-from-top-2 duration-200">
                        <ul className="space-y-1">
                          {item.items?.map((subItem) => (
                            <li key={subItem.label}>
                              <Link href={subItem.href} className="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-gray-100 dark:hover:bg-white/5 group/dItem transition-colors" onClick={() => setActiveDropdown(null)}>
          ...
          <div className="hidden lg:flex items-center gap-6">
            <Link href="/contact" className="group relative px-6 py-2.5 overflow-hidden rounded-full font-semibold text-sm transition-all shadow-lg bg-bvm-blue text-white hover:bg-bvm-blue-dark">
            ...
            {navStructure.map((item) => (
              <div key={item.label} className="border-b border-gray-100 dark:border-white/5 pb-4">
                {item.type ? (
                      ...
                      {item.columns ? (
                        ...
                        item.columns.map(col => (
                          <div key={col.title} className="space-y-2">
                            ...
                            {col.items.map(sub => (
                              <Link key={sub.label} href={sub.href} className="block text-gray-600 dark:text-white/70 py-1" onClick={() => setIsMobileMenuOpen(false)}>{sub.label}</Link>
                            ))}
                          </div>
                        ))
                      ) : (
                  ...
                  item.href && <Link href={item.href} className="block text-xl font-display font-medium text-gray-900 dark:text-white" onClick={() => setIsMobileMenuOpen(false)}>{item.label}</Link>
                )}
              </div>
            ))}
            <Link href="/contact" className="block w-full py-4 text-center bg-bvm-blue text-white rounded-xl font-bold mt-8" onClick={() => setIsMobileMenuOpen(false)}>
              Request a Quote
            </Link>
          </div>
        </div>
